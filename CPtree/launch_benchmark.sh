...
do
    ...
    for data in tic_tac_toe balance_scale  car monk1 ;
    do
    for (( depth=5; depth<=$3; depth++))
    do	
        python3 holdout_experiments_cptree.py --dataset_name=$data --L=$((2 ** depth)) --K=$depth --max_time=$4 --iter=$iter;
    done
    done
done
