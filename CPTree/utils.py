...
def decisionTreeConstraint(K,L,V,max_time,X,Y,classnames,colnames, C,imbalanced=True,min_number=1,precedence=None
    ...
    for e in range(N):
        ...
        for c in range(len(C)):
            r[(e,c)]=model.NewBoolVar('r'+'('+str(e)+','+str(c)+')')
    ...
    for i in range(L):
        ...
        for j in range(K):
            for a in L_i:
                ...
                model.Add(sum(z[(min(i,a),j,max(a,i),q)] for q in range(3))==1)
                for l in range(M):
                    model.Add(sum(y[(i,j,l,p)] for p in range(V))==sum(y[(a,j,l,p)] for p in range(V))).OnlyEnforceIf(z[(min(i,a),j,max(a,i),1)])
            model.Add(sum(z[(min(i,a),j,max(a,i),1)] for a in L_i)>=sum(y[(i,j,l,p)] for l in range(M) for p in range(V)))
        for j in range(K-1):
            for a in L_i:
                model.Add(z[(min(i,a),j,max(a,i),0)]<=sum(y[(i,j+1,l,p)] for l in range(M) for p in range(V)))
    for e in range(N):
    ...
    for e in range(N):
        for c in range(len(C)):
            ...
            for i in range(L):
                ...
                #model.Add(r[(e,c)]>=cl[(i,c)]-Y[e,c]).OnlyEnforceIf(x[(e,i)])
                model.Add(r[(e,c)]<=2-cl[(i,c)]-Y[e,c]).OnlyEnforceIf(x[(e,i)])
    if imbalanced:
        for c in range(len(C)):
    ...
    if must_be_in:
        for b in must_be_in:
            model.Add(sum(y[(i,j,b,p)] for i in range(L) for j in range(K) for p in range(V))>=2)
    ...
    model.Minimize(sum(r[(e,c)] for e in range(N) for c in range(len(C))))
