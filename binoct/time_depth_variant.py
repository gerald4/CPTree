...
#		   "wine"]
datasets = ["tic_tac_toe", "balance_scale", "car", "monk1"]
evaluation_method = "cat_dis_holdout"

for i in range(1,2):
#for filename in glob.glob(os.path.join(path1, '*.train*.csv')):
	for dataset in datasets:
		if dataset in ["monk1", "monk2", "monk3", "spect_heart"] and i>=2:
				pass
		else:
				path1 = f"time_depth_variant/{dataset}_{evaluation_method}_"
				filename=path1+"train_"+str(i)+".csv"
				print(filename)
			     #lcb.main(["-f",filename, "-d", 1, "-t", 900, "-p", 300])

				for depth in range(2,5):
					   lcb.main(["-f",filename, "-d", depth, "-t", 2400, "-p", 150])
