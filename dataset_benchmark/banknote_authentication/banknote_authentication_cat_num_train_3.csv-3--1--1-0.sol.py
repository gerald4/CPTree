...
def predict(row,header):
  if float(row[header["curtosis_of_wavelet"]]) <= 1.93445:
    if float(row[header["skewness_of_wavelet"]]) <= 5.095599999999999:
      if float(row[header["Variance_of_wavelet"]]) <= 2.19455:
        return 1.0
      if float(row[header["Variance_of_wavelet"]]) > 2.19455:
        return 0.0
    if float(row[header["skewness_of_wavelet"]]) > 5.095599999999999:
      if float(row[header["Variance_of_wavelet"]]) <= -2.73785:
        return 1.0
      if float(row[header["Variance_of_wavelet"]]) > -2.73785:
        return 0.0
  if float(row[header["curtosis_of_wavelet"]]) > 1.93445:
    if float(row[header["entropy_of_image"]]) <= -2.20235:
      if float(row[header["curtosis_of_wavelet"]]) <= 8.682500000000001:
        return 0.0
      if float(row[header["curtosis_of_wavelet"]]) > 8.682500000000001:
        return 1.0
    if float(row[header["entropy_of_image"]]) > -2.20235:
      if float(row[header["Variance_of_wavelet"]]) <= -0.458565:
        return 1.0
      if float(row[header["Variance_of_wavelet"]]) > -0.458565:
        return 0.0
