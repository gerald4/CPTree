...
def predict(row,header):
  if float(row[header["Feat21"]]) <= 1.2325:
    if float(row[header["Feat0"]]) <= 5.0745000000000005:
      if float(row[header["Feat37"]]) <= 0.5:
        return 1.0
      if float(row[header["Feat37"]]) > 0.5:
        return 0.0
    if float(row[header["Feat0"]]) > 5.0745000000000005:
      if float(row[header["Feat17"]]) <= 1.1235:
        return 1.0
      if float(row[header["Feat17"]]) > 1.1235:
        return 0.0
  if float(row[header["Feat21"]]) > 1.2325:
    if float(row[header["Feat15"]]) <= 0.5:
      if float(row[header["Feat9"]]) <= 2.5:
        return 0.0
      if float(row[header["Feat9"]]) > 2.5:
        return 1.0
    if float(row[header["Feat15"]]) > 0.5:
      if float(row[header["Feat0"]]) <= 4.667:
        return 1.0
      if float(row[header["Feat0"]]) > 4.667:
        return 0.0
