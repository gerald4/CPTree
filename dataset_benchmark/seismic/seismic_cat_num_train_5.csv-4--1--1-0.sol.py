...
train = "seismic_cat_num_train_5.csv"
test = "seismic_cat_num_test_5.csv"
def predict(row,header):
  if float(row[header["Feat3"]]) <= 433525.0:
    if float(row[header["Feat5"]]) <= 93.5:
      if float(row[header["Feat3"]]) <= 122730.0:
        if float(row[header["Feat3"]]) <= 122220.0:
          return 0.0
        if float(row[header["Feat3"]]) > 122220.0:
          return 1.0
      if float(row[header["Feat3"]]) > 122730.0:
        if float(row[header["Feat3"]]) <= 54690.0:
          return 1.0
        if float(row[header["Feat3"]]) > 54690.0:
          return 0.0
    if float(row[header["Feat5"]]) > 93.5:
      if float(row[header["Feat1_2"]]) <= 0.5:
        ...
        if float(row[header["Feat1_1"]]) > 0.5:
          return 0.0
      if float(row[header["Feat1_2"]]) > 0.5:
        if float(row[header["Feat5"]]) <= 128.5:
          return 1.0
        if float(row[header["Feat5"]]) > 128.5:
          return 0.0
  if float(row[header["Feat3"]]) > 433525.0:
    if float(row[header["Feat0_1"]]) <= 0.5:
      if float(row[header["Feat3"]]) <= 29510.0:
        if float(row[header["Feat3"]]) <= 1877915.0:
          return 0.0
        if float(row[header["Feat3"]]) > 1877915.0:
          return 0.0
      if float(row[header["Feat3"]]) > 29510.0:
