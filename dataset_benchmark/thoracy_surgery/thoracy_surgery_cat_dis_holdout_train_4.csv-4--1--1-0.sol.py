...
train = "thoracy_surgery_cat_dis_holdout_train_4.csv"
test = "thoracy_surgery_cat_dis_holdout_test_4.csv"
def predict(row,header):
  if float(row[header["Weakness.Before_T"]]) <= 0.5:
    ...
    if float(row[header["size.Orign.Tumour_OC13"]]) > 0.5:
      if float(row[header["FVC_0.0"]]) <= 0.5:
        if float(row[header["Perform.Status_PRZ0"]]) <= 0.5:
          return 0.0
        if float(row[header["Perform.Status_PRZ0"]]) > 0.5:
          return 1.0
      if float(row[header["FVC_0.0"]]) > 0.5:
    ...
    if float(row[header["Age_0.0"]]) <= 0.5:
      if float(row[header["Dyspnoea.Before_T"]]) <= 0.5:
        ...
        if float(row[header["Pain.Before_T"]]) > 0.5:
          return 1.0
      if float(row[header["Dyspnoea.Before_T"]]) > 0.5:
        if float(row[header["DGN_DGN1"]]) <= 0.5:
          return 1.0
        if float(row[header["DGN_DGN1"]]) > 0.5:
          return 1.0
    if float(row[header["Age_0.0"]]) > 0.5:
      if float(row[header["FVC_1.0"]]) <= 0.5:
        if float(row[header["size.Orign.Tumour_OC13"]]) <= 0.5:
          return 0.0
        if float(row[header["size.Orign.Tumour_OC13"]]) > 0.5:
          return 0.0
      if float(row[header["FVC_1.0"]]) > 0.5:
        if float(row[header["FEV1_2.0"]]) <= 0.5:
          return 1.0
        if float(row[header["FEV1_2.0"]]) > 0.5:
          return 0.0
