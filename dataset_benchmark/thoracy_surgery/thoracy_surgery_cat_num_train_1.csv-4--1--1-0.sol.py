...
train = "thoracy_surgery_cat_num_train_1.csv"
test = "thoracy_surgery_cat_num_test_1.csv"
    ...
    if float(row[header["Dyspnoea.Before_T"]]) <= 0.5:
      if float(row[header["DGN_DGN2"]]) <= 0.5:
        ...
        if float(row[header["DGN_DGN5"]]) > 0.5:
          return 1.0
      if float(row[header["DGN_DGN2"]]) > 0.5:
        if float(row[header["FVC"]]) <= 3.9699999999999998:
          return 0.0
        if float(row[header["FVC"]]) > 3.9699999999999998:
          return 1.0
    if float(row[header["Dyspnoea.Before_T"]]) > 0.5:
      if float(row[header["FEV1"]]) <= 2.085:
        if float(row[header["FVC"]]) <= 4.665:
          return 1.0
        if float(row[header["FVC"]]) > 4.665:
          return 1.0
      if float(row[header["FEV1"]]) > 2.085:
  ...
  if float(row[header["FVC"]]) > 4.34:
    if float(row[header["Dyspnoea.Before_T"]]) <= 0.5:
      if float(row[header["size.Orign.Tumour_OC14"]]) <= 0.5:
        if float(row[header["Perform.Status_PRZ2"]]) <= 0.5:
          return 0.0
        if float(row[header["Perform.Status_PRZ2"]]) > 0.5:
          return 1.0
      if float(row[header["size.Orign.Tumour_OC14"]]) > 0.5:
        ...
        if float(row[header["Perform.Status_PRZ0"]]) > 0.5:
          return 1.0
    if float(row[header["Dyspnoea.Before_T"]]) > 0.5:
      if float(row[header["FVC"]]) <= 3.875:
        if float(row[header["Age"]]) <= 84.0:
          return 0.0
        if float(row[header["Age"]]) > 84.0:
          return 0.0
      if float(row[header["FVC"]]) > 3.875:
