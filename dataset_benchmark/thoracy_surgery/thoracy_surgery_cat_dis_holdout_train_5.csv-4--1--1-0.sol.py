...
train = "thoracy_surgery_cat_dis_holdout_train_5.csv"
test = "thoracy_surgery_cat_dis_holdout_test_5.csv"
    ...
    if float(row[header["size.Orign.Tumour_OC14"]]) > 0.5:
      if float(row[header["Age_0.0"]]) <= 0.5:
        ...
        if float(row[header["FVC_1.0"]]) > 0.5:
          return 0.0
      if float(row[header["Age_0.0"]]) > 0.5:
        if float(row[header["DGN_DGN3"]]) <= 0.5:
          return 0.0
        if float(row[header["DGN_DGN3"]]) > 0.5:
          return 1.0
  if float(row[header["DGN_DGN5"]]) > 0.5:
    ...
    if float(row[header["FVC_1.0"]]) > 0.5:
      if float(row[header["size.Orign.Tumour_OC12"]]) <= 0.5:
        ...
        if float(row[header["Perform.Status_PRZ0"]]) > 0.5:
          return 0.0
      if float(row[header["size.Orign.Tumour_OC12"]]) > 0.5:
        if float(row[header["Smoking_T"]]) <= 0.5:
          return 1.0
        if float(row[header["Smoking_T"]]) > 0.5:
          return 0.0
