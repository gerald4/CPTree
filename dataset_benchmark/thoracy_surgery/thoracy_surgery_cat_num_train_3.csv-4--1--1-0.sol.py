...
train = "thoracy_surgery_cat_num_train_3.csv"
test = "thoracy_surgery_cat_num_test_3.csv"
    ...
    if float(row[header["DGN_DGN5"]]) <= 0.5:
      if float(row[header["FVC"]]) <= 4.34:
        ...
        if float(row[header["DGN_DGN8"]]) > 0.5:
          return 1.0
      if float(row[header["FVC"]]) > 4.34:
        if float(row[header["FEV1"]]) <= 3.0300000000000002:
          return 1.0
        if float(row[header["FEV1"]]) > 3.0300000000000002:
          return 0.0
    if float(row[header["DGN_DGN5"]]) > 0.5:
      if float(row[header["FVC"]]) <= 2.875:
        if float(row[header["Hamemoptysis.Before_T"]]) <= 0.5:
          return 1.0
        if float(row[header["Hamemoptysis.Before_T"]]) > 0.5:
          return 0.0
      if float(row[header["FVC"]]) > 2.875:
        if float(row[header["FEV1"]]) <= 2.135:
          return 1.0
        if float(row[header["FEV1"]]) > 2.135:
          return 0.0
  if float(row[header["Type2.Diabetes_T"]]) > 0.5:
    if float(row[header["FEV1"]]) <= 2.759999990463257:
      if float(row[header["Hamemoptysis.Before_T"]]) <= 0.5:
        if float(row[header["Age"]]) <= 58.5:
          return 1.0
        if float(row[header["Age"]]) > 58.5:
          return 0.0
      if float(row[header["Hamemoptysis.Before_T"]]) > 0.5:
