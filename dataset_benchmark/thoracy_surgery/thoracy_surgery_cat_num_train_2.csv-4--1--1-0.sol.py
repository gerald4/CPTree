...
train = "thoracy_surgery_cat_num_train_2.csv"
test = "thoracy_surgery_cat_num_test_2.csv"
  ...
  if float(row[header["Perform.Status_PRZ2"]]) <= 0.5:
    if float(row[header["DGN_DGN5"]]) <= 0.5:
      if float(row[header["Perform.Status_PRZ1"]]) <= 0.5:
        if float(row[header["FEV1"]]) <= 1.38:
          return 1.0
        if float(row[header["FEV1"]]) > 1.38:
          return 0.0
      if float(row[header["Perform.Status_PRZ1"]]) > 0.5:
        if float(row[header["FVC"]]) <= 4.98:
          return 0.0
        if float(row[header["FVC"]]) > 4.98:
          return 0.0
    if float(row[header["DGN_DGN5"]]) > 0.5:
      ...
      if float(row[header["Pain.Before_T"]]) > 0.5:
        ...
        if float(row[header["FVC"]]) > 4.98:
          return 0.0
  if float(row[header["Perform.Status_PRZ2"]]) > 0.5:
    if float(row[header["FVC"]]) <= 2.3899999999999997:
      if float(row[header["FEV1"]]) <= 3.54:
        if float(row[header["FVC"]]) <= 4.98:
          return 0.0
        if float(row[header["FVC"]]) > 4.98:
          return 0.0
      if float(row[header["FEV1"]]) > 3.54:
        ...
        if float(row[header["FVC"]]) > 4.98:
          return 0.0
    if float(row[header["FVC"]]) > 2.3899999999999997:
      if float(row[header["FVC"]]) <= 3.02:
        if float(row[header["DGN_DGN2"]]) <= 0.5:
          return 1.0
        if float(row[header["DGN_DGN2"]]) > 0.5:
          return 0.0
      if float(row[header["FVC"]]) > 3.02:
        if float(row[header["Age"]]) <= 79.5:
          return 0.0
        if float(row[header["Age"]]) > 79.5:
          return 1.0
