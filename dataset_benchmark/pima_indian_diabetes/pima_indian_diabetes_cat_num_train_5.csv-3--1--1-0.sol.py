...
def predict(row,header):
  if float(row[header["DiabetesPedigreeFunction"]]) <= 0.9045000000000001:
    if float(row[header["BMI"]]) <= 34.150000000000006:
      if float(row[header["Glucose"]]) <= 147.5:
        return 0.0
      if float(row[header["Glucose"]]) > 147.5:
        return 1.0
    if float(row[header["BMI"]]) > 34.150000000000006:
      if float(row[header["Glucose"]]) <= 128.5:
        return 0.0
      if float(row[header["Glucose"]]) > 128.5:
        return 1.0
  if float(row[header["DiabetesPedigreeFunction"]]) > 0.9045000000000001:
    if float(row[header["Pregnancies"]]) <= 3.5:
      if float(row[header["Glucose"]]) <= 122.5:
        return 0.0
      if float(row[header["Glucose"]]) > 122.5:
        return 1.0
    if float(row[header["Pregnancies"]]) > 3.5:
      if float(row[header["DiabetesPedigreeFunction"]]) <= 1.426:
        return 1.0
      if float(row[header["DiabetesPedigreeFunction"]]) > 1.426:
        return 0.0
