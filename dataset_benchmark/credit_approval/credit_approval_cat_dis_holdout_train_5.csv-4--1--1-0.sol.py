...
train = "credit_approval_cat_dis_holdout_train_5.csv"
test = "credit_approval_cat_dis_holdout_test_5.csv"
def predict(row,header):
  if float(row[header["A7_j"]]) <= 0.5:
    if float(row[header["A9_t"]]) <= 0.5:
      if float(row[header["A5_gg"]]) <= 0.5:
        if float(row[header["A4_u"]]) <= 0.5:
          return 1.0
        if float(row[header["A4_u"]]) > 0.5:
          return 1.0
      if float(row[header["A5_gg"]]) > 0.5:
        if float(row[header["A8_2.0"]]) <= 0.5:
          return 0.0
        if float(row[header["A8_2.0"]]) > 0.5:
          return 0.0
    if float(row[header["A9_t"]]) > 0.5:
      if float(row[header["A14_1.0"]]) <= 0.5:
        ...
        if float(row[header["A14_1.0"]]) > 0.5:
          return 1.0
      if float(row[header["A14_1.0"]]) > 0.5:
        if float(row[header["A3_0.0"]]) <= 0.5:
          return 0.0
        if float(row[header["A3_0.0"]]) > 0.5:
          return 1.0
  if float(row[header["A7_j"]]) > 0.5:
    if float(row[header["A9_t"]]) <= 0.5:
      if float(row[header["A6_i"]]) <= 0.5:
        if float(row[header["A1_b"]]) <= 0.5:
          return 1.0
        if float(row[header["A1_b"]]) > 0.5:
          return 0.0
      if float(row[header["A6_i"]]) > 0.5:
