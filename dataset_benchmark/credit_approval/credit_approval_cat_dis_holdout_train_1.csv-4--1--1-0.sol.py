...
train = "credit_approval_cat_dis_holdout_train_1.csv"
test = "credit_approval_cat_dis_holdout_test_1.csv"
def predict(row,header):
  if float(row[header["A6_aa"]]) <= 0.5:
    ...
    if float(row[header["A3_0.0"]]) > 0.5:
      if float(row[header["A14_1.0"]]) <= 0.5:
        ...
        if float(row[header["A9_t"]]) > 0.5:
          return 0.0
      if float(row[header["A14_1.0"]]) > 0.5:
        if float(row[header["A6_r"]]) <= 0.5:
          return 1.0
        if float(row[header["A6_r"]]) > 0.5:
          return 0.0
  if float(row[header["A6_aa"]]) > 0.5:
    if float(row[header["A1_b"]]) <= 0.5:
      if float(row[header["A7_v"]]) <= 0.5:
        if float(row[header["A4_l"]]) <= 0.5:
          return 1.0
        if float(row[header["A4_l"]]) > 0.5:
          return 1.0
      if float(row[header["A7_v"]]) > 0.5:
        ...
        if float(row[header["A9_t"]]) > 0.5:
          return 0.0
    if float(row[header["A1_b"]]) > 0.5:
      if float(row[header["A3_1.0"]]) <= 0.5:
