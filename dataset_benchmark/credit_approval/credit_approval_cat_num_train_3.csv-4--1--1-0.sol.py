...
train = "credit_approval_cat_num_train_3.csv"
test = "credit_approval_cat_num_test_3.csv"
def predict(row,header):
  if float(row[header["A7_ff"]]) <= 0.5:
    if float(row[header["A13_s"]]) <= 0.5:
      if float(row[header["A5_gg"]]) <= 0.5:
        if float(row[header["A9_t"]]) <= 0.5:
          return 1.0
        if float(row[header["A9_t"]]) > 0.5:
          return 0.0
      if float(row[header["A5_gg"]]) > 0.5:
        if float(row[header["A7_h"]]) <= 0.5:
          return 0.0
        if float(row[header["A7_h"]]) > 0.5:
          return 1.0
    if float(row[header["A13_s"]]) > 0.5:
      if float(row[header["A9_t"]]) <= 0.5:
        if float(row[header["A14"]]) <= 467.5:
          return 1.0
        if float(row[header["A14"]]) > 467.5:
          return 0.0
      if float(row[header["A9_t"]]) > 0.5:
        if float(row[header["A6_m"]]) <= 0.5:
          return 0.0
        if float(row[header["A6_m"]]) > 0.5:
          return 1.0
  if float(row[header["A7_ff"]]) > 0.5:
    if float(row[header["A6_c"]]) <= 0.5:
      if float(row[header["A5_g"]]) <= 0.5:
        if float(row[header["A4_y"]]) <= 0.5:
          return 0.0
        if float(row[header["A4_y"]]) > 0.5:
          return 1.0
      if float(row[header["A5_g"]]) > 0.5:
        if float(row[header["A3"]]) <= 9.085:
          return 1.0
        if float(row[header["A3"]]) > 9.085:
          return 0.0
    if float(row[header["A6_c"]]) > 0.5:
      if float(row[header["A2"]]) <= 30.21:
        if float(row[header["A8"]]) <= 13.4375:
          return 0.0
        if float(row[header["A8"]]) > 13.4375:
          return 0.0
      if float(row[header["A2"]]) > 30.21:
        if float(row[header["A2"]]) <= 39.125:
          return 1.0
        if float(row[header["A2"]]) > 39.125:
          return 1.0
