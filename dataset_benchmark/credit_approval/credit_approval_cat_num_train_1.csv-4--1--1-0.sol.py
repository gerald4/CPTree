...
train = "credit_approval_cat_num_train_1.csv"
test = "credit_approval_cat_num_test_1.csv"
def predict(row,header):
  if float(row[header["A4_y"]]) <= 0.5:
    if float(row[header["A2"]]) <= 71.46000000000001:
      if float(row[header["A9_t"]]) <= 0.5:
        if float(row[header["A15"]]) <= 5664.5:
          return 1.0
        if float(row[header["A15"]]) > 5664.5:
          return 0.0
      if float(row[header["A9_t"]]) > 0.5:
        ...
        if float(row[header["A4_l"]]) > 0.5:
          return 0.0
    if float(row[header["A2"]]) > 71.46000000000001:
      if float(row[header["A2"]]) <= 40.705:
        if float(row[header["A4_l"]]) <= 0.5:
          return 1.0
        if float(row[header["A4_l"]]) > 0.5:
          return 1.0
      if float(row[header["A2"]]) > 40.705:
        if float(row[header["A2"]]) <= 30.25:
          return 0.0
        if float(row[header["A2"]]) > 30.25:
          return 0.0
  if float(row[header["A4_y"]]) > 0.5:
    if float(row[header["A11"]]) <= 5.5:
      if float(row[header["A9_t"]]) <= 0.5:
        if float(row[header["A2"]]) <= 71.46000000000001:
          return 1.0
        if float(row[header["A2"]]) > 71.46000000000001:
          return 1.0
      if float(row[header["A9_t"]]) > 0.5:
        if float(row[header["A15"]]) <= 264.5:
          return 1.0
        if float(row[header["A15"]]) > 264.5:
          return 0.0
    if float(row[header["A11"]]) > 5.5:
      if float(row[header["A7_z"]]) <= 0.5:
        if float(row[header["A2"]]) <= 58.0:
          return 0.0
        if float(row[header["A2"]]) > 58.0:
          return 1.0
      if float(row[header["A7_z"]]) > 0.5:
        if float(row[header["A5_p"]]) <= 0.5:
          return 1.0
        if float(row[header["A5_p"]]) > 0.5:
          return 0.0
