...
def predict(row,header):
  if float(row[header["Feat21"]]) <= 0.465405:
    if float(row[header["Feat4"]]) <= 0.10086:
      if float(row[header["Feat4"]]) <= 0.02313:
        return 0.0
      if float(row[header["Feat4"]]) > 0.02313:
        return 1.0
    if float(row[header["Feat4"]]) > 0.10086:
      if float(row[header["Feat2"]]) <= 0.19028:
        return 0.0
      if float(row[header["Feat2"]]) > 0.19028:
        return 1.0
  if float(row[header["Feat21"]]) > 0.465405:
    if float(row[header["Feat24"]]) <= 0.999495:
      if float(row[header["Feat11"]]) <= 0.112695:
        return 0.0
      if float(row[header["Feat11"]]) > 0.112695:
        return 1.0
    if float(row[header["Feat24"]]) > 0.999495:
      if float(row[header["Feat16"]]) <= -0.032595:
        return 1.0
      if float(row[header["Feat16"]]) > -0.032595:
        return 0.0
