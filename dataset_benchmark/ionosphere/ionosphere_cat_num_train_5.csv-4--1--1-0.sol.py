...
train = "ionosphere_cat_num_train_5.csv"
test = "ionosphere_cat_num_test_5.csv"
def predict(row,header):
  if float(row[header["Feat32"]]) <= 0.9946699999999999:
    if float(row[header["Feat26"]]) <= 0.9999450147151947:
      if float(row[header["Feat33"]]) <= 0.950980007648468:
        if float(row[header["Feat4"]]) <= 0.041440000000000005:
          return 0.0
        if float(row[header["Feat4"]]) > 0.041440000000000005:
          return 1.0
      if float(row[header["Feat33"]]) > 0.950980007648468:
        if float(row[header["Feat22"]]) <= 0.0195:
          return 0.0
        if float(row[header["Feat22"]]) > 0.0195:
          return 1.0
    if float(row[header["Feat26"]]) > 0.9999450147151947:
      if float(row[header["Feat11"]]) <= 0.021065:
        if float(row[header["Feat18"]]) <= 0.998845:
          return 0.0
        if float(row[header["Feat18"]]) > 0.998845:
          return 1.0
      if float(row[header["Feat11"]]) > 0.021065:
        if float(row[header["Feat8"]]) <= 0.99919:
          return 0.0
        if float(row[header["Feat8"]]) > 0.99919:
          return 0.0
  if float(row[header["Feat32"]]) > 0.9946699999999999:
    if float(row[header["Feat5"]]) <= -0.74729:
      if float(row[header["Feat6"]]) <= 0.97555:
        if float(row[header["Feat19"]]) <= 0.779445:
          return 0.0
        if float(row[header["Feat19"]]) > 0.779445:
          return 0.0
      if float(row[header["Feat6"]]) > 0.97555:
        ...
        if float(row[header["Feat0"]]) > 0.5:
          return 0.0
    if float(row[header["Feat5"]]) > -0.74729:
      if float(row[header["Feat3"]]) <= -0.6800599992275238:
        if float(row[header["Feat0"]]) <= 0.5:
          return 0.0
        if float(row[header["Feat0"]]) > 0.5:
          return 0.0
      if float(row[header["Feat3"]]) > -0.6800599992275238:
        if float(row[header["Feat30"]]) <= 0.7510749999999999:
          return 0.0
        if float(row[header["Feat30"]]) > 0.7510749999999999:
          return 1.0
