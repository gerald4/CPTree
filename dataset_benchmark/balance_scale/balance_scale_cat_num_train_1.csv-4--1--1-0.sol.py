...
train = "balance_scale_cat_num_train_1.csv"
test = "balance_scale_cat_num_test_1.csv"
    ...
    if float(row[header["Left-Weight_1"]]) > 0.5:
      if float(row[header["Left-Distance_1"]]) <= 0.5:
        ...
        if float(row[header["Right-Weight_1"]]) > 0.5:
          return 1.0
      if float(row[header["Left-Distance_1"]]) > 0.5:
        if float(row[header["Left-Weight_3"]]) <= 0.5:
          return 2.0
        if float(row[header["Left-Weight_3"]]) > 0.5:
          return 1.0
  if float(row[header["Left-Distance_2"]]) > 0.5:
    ...
    if float(row[header["Right-Distance_1"]]) > 0.5:
      if float(row[header["Right-Weight_1"]]) <= 0.5:
        ...
        if float(row[header["Left-Weight_1"]]) > 0.5:
          return 2.0
      if float(row[header["Right-Weight_1"]]) > 0.5:
        if float(row[header["Right-Distance_3"]]) <= 0.5:
          return 1.0
        if float(row[header["Right-Distance_3"]]) > 0.5:
          return 2.0
