...
train = "bank_marketing_cat_dis_holdout_train_2.csv"
test = "bank_marketing_cat_dis_holdout_test_2.csv"
  ...
  if float(row[header["education_basic.4y"]]) <= 0.5:
    if float(row[header["month_dec"]]) <= 0.5:
      ...
      if float(row[header["poutcome_success"]]) > 0.5:
        return 1.0
    if float(row[header["month_dec"]]) > 0.5:
      if float(row[header["day_of_week_wed"]]) <= 0.5:
        return 0.0
      if float(row[header["day_of_week_wed"]]) > 0.5:
        return 1.0
  if float(row[header["education_basic.4y"]]) > 0.5:
    if float(row[header["nr.employed_0.0"]]) <= 0.5:
      if float(row[header["marital_divorced"]]) <= 0.5:
        return 0.0
      if float(row[header["marital_divorced"]]) > 0.5:
        return 0.0
    if float(row[header["nr.employed_0.0"]]) > 0.5:
      if float(row[header["cons.price.idx_1.0"]]) <= 0.5:
        return 0.0
      if float(row[header["cons.price.idx_1.0"]]) > 0.5:
        return 1.0
