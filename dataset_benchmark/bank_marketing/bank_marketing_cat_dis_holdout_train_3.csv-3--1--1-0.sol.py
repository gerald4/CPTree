...
def predict(row,header):
  if float(row[header["emp.var.rate_1.0"]]) <= 0.5:
    if float(row[header["month_dec"]]) <= 0.5:
      ...
      if float(row[header["poutcome_success"]]) > 0.5:
        return 1.0
    if float(row[header["month_dec"]]) > 0.5:
      if float(row[header["job_services"]]) <= 0.5:
        return 1.0
      if float(row[header["job_services"]]) > 0.5:
        return 1.0
  if float(row[header["emp.var.rate_1.0"]]) > 0.5:
    if float(row[header["job_self-employed"]]) <= 0.5:
      if float(row[header["education_university.degree"]]) <= 0.5:
        return 0.0
      if float(row[header["education_university.degree"]]) > 0.5:
        return 0.0
    if float(row[header["job_self-employed"]]) > 0.5:
