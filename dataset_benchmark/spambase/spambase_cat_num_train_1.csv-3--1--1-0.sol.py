...
def predict(row,header):
  if float(row[header["Feat41"]]) <= 0.365:
    if float(row[header["Feat52"]]) <= 0.012:
      if float(row[header["Feat6"]]) <= 0.255:
        return 0.0
      if float(row[header["Feat6"]]) > 0.255:
        return 1.0
    if float(row[header["Feat52"]]) > 0.012:
      if float(row[header["Feat24"]]) <= 0.215:
        return 1.0
      if float(row[header["Feat24"]]) > 0.215:
        return 0.0
  if float(row[header["Feat41"]]) > 0.365:
    if float(row[header["Feat4"]]) <= 1.185:
      if float(row[header["Feat6"]]) <= 0.655:
        return 0.0
      if float(row[header["Feat6"]]) > 0.655:
        return 1.0
    if float(row[header["Feat4"]]) > 1.185:
