...
def predict(row,header):
  if float(row[header["Feat15"]]) <= 0.395:
    if float(row[header["Feat55"]]) <= 55.5:
      if float(row[header["Feat52"]]) <= 0.055499999999999994:
        return 0.0
      if float(row[header["Feat52"]]) > 0.055499999999999994:
        return 1.0
    if float(row[header["Feat55"]]) > 55.5:
      if float(row[header["Feat26"]]) <= 0.005:
        return 1.0
      if float(row[header["Feat26"]]) > 0.005:
        return 0.0
  if float(row[header["Feat15"]]) > 0.395:
    if float(row[header["Feat41"]]) <= 0.035:
      if float(row[header["Feat31"]]) <= 0.475:
        return 1.0
      if float(row[header["Feat31"]]) > 0.475:
        return 0.0
    if float(row[header["Feat41"]]) > 0.035:
      if float(row[header["Feat40"]]) <= 0.115:
        return 0.0
      if float(row[header["Feat40"]]) > 0.115:
        return 0.0
