...
def predict(row,header):
  if float(row[header["Petal_length"]]) <= 2.45:
    if float(row[header["Sepal_length"]]) <= 7.1:
      ...
      if float(row[header["Petal_width"]]) > 1.550000011920929:
        ...
        if float(row[header["Sepal_length"]]) > 6.85:
          return 2.0
    if float(row[header["Sepal_length"]]) > 7.1:
      if float(row[header["Sepal_length"]]) <= 5.65:
      ...
      if float(row[header["Sepal_length"]]) > 5.65:
        ...
        if float(row[header["Sepal_length"]]) > 5.95:
          return 2.0
  if float(row[header["Petal_length"]]) > 2.45:
    if float(row[header["Sepal_width"]]) <= 3.100000023841858:
      if float(row[header["Petal_length"]]) <= 4.95:
        if float(row[header["Petal_width"]]) <= 1.55:
          return 1.0
        if float(row[header["Petal_width"]]) > 1.55:
          return 2.0
      if float(row[header["Petal_length"]]) > 4.95:
        if float(row[header["Sepal_width"]]) <= 2.45:
          return 1.0
        if float(row[header["Sepal_width"]]) > 2.45:
          return 2.0
    if float(row[header["Sepal_width"]]) > 3.100000023841858:
      if float(row[header["Sepal_width"]]) <= 3.55:
        if float(row[header["Petal_length"]]) <= 5.0:
          return 1.0
        if float(row[header["Petal_length"]]) > 5.0:
          return 2.0
      if float(row[header["Sepal_width"]]) > 3.55:
