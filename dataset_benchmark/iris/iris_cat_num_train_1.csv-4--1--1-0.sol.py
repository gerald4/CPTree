...
train = "iris_cat_num_train_1.csv"
test = "iris_cat_num_test_1.csv"
def predict(row,header):
  if float(row[header["Petal_length"]]) <= 5.05:
    if float(row[header["Petal_length"]]) <= 4.950000047683716:
      if float(row[header["Petal_width"]]) <= 1.75:
        if float(row[header["Petal_width"]]) <= 0.800000011920929:
          return 0.0
        if float(row[header["Petal_width"]]) > 0.800000011920929:
          return 1.0
      if float(row[header["Petal_width"]]) > 1.75:
        if float(row[header["Sepal_width"]]) <= 3.1500000000000004:
          return 2.0
        if float(row[header["Sepal_width"]]) > 3.1500000000000004:
          return 1.0
    if float(row[header["Petal_length"]]) > 4.950000047683716:
      if float(row[header["Petal_width"]]) <= 1.85:
        ...
        if float(row[header["Petal_width"]]) > 1.65:
          return 1.0
      if float(row[header["Petal_width"]]) > 1.85:
        if float(row[header["Sepal_width"]]) <= 3.9:
          return 2.0
        if float(row[header["Sepal_width"]]) > 3.9:
          return 2.0
  if float(row[header["Petal_length"]]) > 5.05:
    if float(row[header["Sepal_length"]]) <= 5.45:
      if float(row[header["Sepal_length"]]) <= 7.050000000000001:
        if float(row[header["Sepal_length"]]) <= 7.050000000000001:
          return 2.0
        if float(row[header["Sepal_length"]]) > 7.050000000000001:
          return 2.0
      if float(row[header["Sepal_length"]]) > 7.050000000000001:
        if float(row[header["Sepal_length"]]) <= 5.05:
          return 2.0
        if float(row[header["Sepal_length"]]) > 5.05:
          return 2.0
    if float(row[header["Sepal_length"]]) > 5.45:
      if float(row[header["Sepal_length"]]) <= 5.95:
        if float(row[header["Sepal_length"]]) <= 5.45:
          return 1.0
        if float(row[header["Sepal_length"]]) > 5.45:
          return 2.0
      if float(row[header["Sepal_length"]]) > 5.95:
        if float(row[header["Sepal_length"]]) <= 6.05:
          return 1.0
        if float(row[header["Sepal_length"]]) > 6.05:
          return 2.0
