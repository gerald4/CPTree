...
train = "post_operative_patient_cat_num_train_2.csv"
test = "post_operative_patient_cat_num_test_2.csv"
  ...
  if float(row[header["BP-STBL_mod-stable"]]) <= 0.5:
    if float(row[header["BP-STBL_unstable"]]) <= 0.5:
      if float(row[header["L-CORE_low"]]) <= 0.5:
        if float(row[header["L-CORE_mid"]]) <= 0.5:
          return 2.0
        if float(row[header["L-CORE_mid"]]) > 0.5:
          return 0.0
      if float(row[header["L-CORE_low"]]) > 0.5:
        if float(row[header["L-SURF_mid"]]) <= 0.5:
          return 0.0
        if float(row[header["L-SURF_mid"]]) > 0.5:
          return 2.0
    if float(row[header["BP-STBL_unstable"]]) > 0.5:
      if float(row[header["L-SURF_low"]]) <= 0.5:
        if float(row[header["COMFORT_15.0"]]) <= 0.5:
          return 0.0
        if float(row[header["COMFORT_15.0"]]) > 0.5:
          return 0.0
      if float(row[header["L-SURF_low"]]) > 0.5:
