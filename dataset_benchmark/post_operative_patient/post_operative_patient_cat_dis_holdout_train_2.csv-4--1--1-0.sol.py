...
train = "post_operative_patient_cat_dis_holdout_train_2.csv"
test = "post_operative_patient_cat_dis_holdout_test_2.csv"
def predict(row,header):
  if float(row[header["BP-STBL_mod-stable"]]) <= 0.5:
    ...
    if float(row[header["BP-STBL_unstable"]]) > 0.5:
      if float(row[header["L-SURF_low"]]) <= 0.5:
        if float(row[header["CORE-STBL_mod-stable"]]) <= 0.5:
          return 0.0
        if float(row[header["CORE-STBL_mod-stable"]]) > 0.5:
          return 0.0
      if float(row[header["L-SURF_low"]]) > 0.5:
  ...
  if float(row[header["BP-STBL_mod-stable"]]) > 0.5:
    ...
    if float(row[header["L-BP_mid"]]) > 0.5:
      if float(row[header["L-SURF_mid"]]) <= 0.5:
        if float(row[header["L-02_good"]]) <= 0.5:
          return 2.0
        if float(row[header["L-02_good"]]) > 0.5:
          return 0.0
      if float(row[header["L-SURF_mid"]]) > 0.5:
