...
train = "post_operative_patient_cat_dis_holdout_train_4.csv"
test = "post_operative_patient_cat_dis_holdout_test_4.csv"
def predict(row,header):
  if float(row[header["BP-STBL_unstable"]]) <= 0.5:
    if float(row[header["L-SURF_high"]]) <= 0.5:
      if float(row[header["BP-STBL_mod-stable"]]) <= 0.5:
        if float(row[header["CORE-STBL_unstable"]]) <= 0.5:
          return 0.0
        if float(row[header["CORE-STBL_unstable"]]) > 0.5:
          return 2.0
      if float(row[header["BP-STBL_mod-stable"]]) > 0.5:
        if float(row[header["L-BP_high"]]) <= 0.5:
          return 2.0
        if float(row[header["L-BP_high"]]) > 0.5:
          return 0.0
    if float(row[header["L-SURF_high"]]) > 0.5:
      if float(row[header["SURF-STBL_unstable"]]) <= 0.5:
        if float(row[header["BP-STBL_mod-stable"]]) <= 0.5:
          return 2.0
        if float(row[header["BP-STBL_mod-stable"]]) > 0.5:
          return 0.0
      if float(row[header["SURF-STBL_unstable"]]) > 0.5:
        if float(row[header["COMFORT_10.0"]]) <= 0.5:
          return 2.0
        if float(row[header["COMFORT_10.0"]]) > 0.5:
          return 0.0
  if float(row[header["BP-STBL_unstable"]]) > 0.5:
    if float(row[header["L-CORE_low"]]) <= 0.5:
      if float(row[header["L-BP_mid"]]) <= 0.5:
        if float(row[header["COMFORT_5.0"]]) <= 0.5:
          return 0.0
        if float(row[header["COMFORT_5.0"]]) > 0.5:
          return 0.0
      if float(row[header["L-BP_mid"]]) > 0.5:
